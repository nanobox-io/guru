...
export default {
  ...
  data() {
    let brain = new Brain(this.model)
    let obj =  {
  ...
  watch:{
    loggedIn(isLoggedIn){
      this.currentPage = (isLoggedIn)? this.brain.sequence.currentItem : 'account';
    }
  }
}
</script>
...
<template lang="pug">
  .guru(v-bind:class="{account:currentPage == 'account'}")
    ...
    finalize(:model="model" v-if="currentPage == 'finalize'" @change="currentPage = arguments[0]" @prev="prevSlide" key="finalize" v-bind:class="{first:firstItem == 'finalize'}" )
</template>
